-- Commercial exception windows, auditable feature overrides and licensed
-- background catalog. Idempotent and safe to run after migration 025.
...
update public.tshow_account_entitlement_history set reason = 'Migración de historial sin motivo registrado' where reason is null or btrim(reason) = '';
alter table public.tshow_account_entitlement_history alter column reason set not null;
alter table public.tshow_account_entitlement_history alter column new_plan drop not null;
...
create policy tshow_background_library_read on public.tshow_background_library for select to authenticated using (active = true);
...
on conflict (background_key) do update set asset_url = excluded.asset_url, alt_text = excluded.alt_text;
