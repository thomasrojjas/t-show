...
test('artist appearance transitions retain an isolated audit history', () => {
  ...
  assert.match(workspace, /capabilities.editOperational/);
  assert.match(route, /areaUpdate/);
  ...
  assert.match(route, /El artista no pertenece a este evento/);
  assert.match(route, /blockBelongsToProject/);
});
...
test('offline cache invalidates stale assets and includes the operational shell', () => {
  ...
  assert.match(serviceWorker, /tshow-static-v4/);
