...
(async () => {
  ...
  try {
    ...
    await page.getByRole('tab', { name: 'Preparación' }).click(); await page.waitForFunction(() => document.querySelector('.operational-status')?.textContent.includes('Listo')); assert(await page.locator('[data-operational-readiness]').count()); assert((await page.locator('.operational-row').first().innerText()).includes('Actualizado')); assert((await page.locator('.operational-row').first().innerText()).includes('Responsable/operador'));
