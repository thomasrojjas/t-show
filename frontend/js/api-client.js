const ApiClient = {
    ...
    async getAllProjects() { const result = await this.request('/projects'); const data = {}; result.data.forEach(project => { data[project.id] = { ...project.payload, id: project.id, eventName: project.event_name, updatedAt: project.updated_at, documentVersion: project.document_version, ownerId: project.owner_id, coverKey: project.cover_key, permission: project.member_role || 'owner' }; }); return { source: 'api', data, meta: result.meta || { ownedCount: 0, limit: 1, remaining: 1, plan: 'free', subscriptionStatus: 'free' } }; },
    async getProject(id) { const result = await this.request(`/projects/${encodeURIComponent(id)}`); return { ...result.data.payload, id: result.data.id, eventName: result.data.event_name, documentVersion: result.data.document_version, coverKey: result.data.cover_key, permission: result.data.permission }; },
