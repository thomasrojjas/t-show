(function(){
  ...
  async function loadSummary(){
    ...
    try{const result=await ApiClient.getAllProjects();const projects=Object.values(result.data||{});sync.textContent='Servidor sincronizado';box.innerHTML=projects.length?projects.map((p,index)=>`<div class="shell-project-row"><span class="shell-row-index">${String(index+1).padStart(2,'0')}</span><a href="#" data-project="${p.id}" data-name="${String(p.eventName||'Proyecto').replace(/"/g,'&quot;')}">${p.eventName||'Proyecto sin nombre'}</a><span class="shell-muted">${p.updatedAt?new Date(p.updatedAt).toLocaleDateString('es-CL'):''}</span></div>`).join(''):'<p class="shell-muted">No hay proyectos disponibles.</p>';if(currentProject){const p=projects.find(item=>item.id===currentProject);if(p){currentProjectName=p.eventName;updateProjectUI();}}}catch(error){sync.textContent='Sin conexión';box.innerHTML=`<div class="shell-message">${error.message||'No se pudieron cargar los proyectos.'}</div>`;}
  }
  ...
  async function loadTeam(){
    ...
    try{const result=await Auth.api(`/api/projects/${encodeURIComponent(currentProject)}/members`);status.textContent='Colaboradores y permisos del proyecto.';box.innerHTML=(result.data||[]).map((m,index)=>`<div class="shell-project-row"><span class="shell-row-index">${String(index+1).padStart(2,'0')}</span><span>${m.profiles?.first_name||''} ${m.profiles?.last_name||m.profiles?.email||''}</span><span class="shell-role">${m.role==='editor'?'Director':'Observador'}</span></div>`).join('')||'<p class="shell-muted">Sin colaboradores todavía.</p>';}catch(error){status.textContent=error.message;}
  }
